...
from topicsync.server.client_manager import Client, ClientCommProtocol, ClientManager
from topicsync.service import Service
from topicsync.state_machine.state_machine import (
    ALREADY_LOGGED_ERROR_NOTE,
    StateMachine,
    Transition,
)
from topicsync.topic import DictTopic, EventTopic, Topic
...
class TopicsyncServer:
    ...
    def __init__(self, transition_callback=lambda transition: None) -> None:
        self.debug = os.environ.get("DEBUG") is not None and (
    ...
    def _initialize(self, debugger, transition_callback):
        ...
        self._topic_list = self._state_machine.add_topic(
            "_topicsync/topic_list",
            DictTopic,
            is_stateful=True,
            init_value={
                "_topicsync/topic_list": {
    ...
    def _add_topic_raw(self, topic_name, props):
        if props.get("is_restore", False):
            ...
            self._state_machine.add_topic_s(
                topic_name,
                props["type"],
                props["is_stateful"],
                props["boundary_value"],
                props["order_strict"],
            )
    ...
    def _handle_action(
        self, sender: Client, commands: list[dict[str, Any]], action_id: str
    ):
        self._action_source = sender.id
        try:
            with self._state_machine.record(
                action_source=sender.id, action_id=action_id
            ):
    ...
    def register_service(
        self, service_name: str, callback: Callable, pass_sender=False
    ):
        """
        Register a service
    ...
    def topic(self, topic_name, type: type[T] = Topic) -> T:
        ...
        if self._state_machine.has_topic(topic_name):
            ...
            assert (
                type is Topic or topic.get_type_name() == type.get_type_name()
            ), f"Topic {topic_name} is of type {topic.get_type_name()} but {type.get_type_name()} was requested"
            return topic  # type: ignore
        else:
    ...
    def add_topic(
        ...
        return self._add_topic_to_list(
            topic_name,
            type,
            {
                "type": type.get_type_name(),
                "boundary_value": init_value,
