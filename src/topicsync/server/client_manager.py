...
class Client:
    def __init__(
        self,
        id,
        comm: ClientCommProtocol,
        sending_queue: asyncio.Queue[Tuple["Client", Tuple, Dict]],
    ):
        self.id = id
        self._comm = comm
...
class ClientManager:
    def __init__(self, state_machine: StateMachine) -> None:
        ...
        self._message_handlers: Dict[str, Callable[..., None | Awaitable[None]]] = {
    ...
    async def handle_client(self, client_comm: ClientCommProtocol):
        ...
        client = self._clients[client_id] = Client(
            client_id, client_comm, self._sending_queue
        )
        ...
        try:
            ...
            async for message in client.messages:
                ...
                try:
                    return_value = self._message_handlers[message_type](
                        sender=client, **args
                    )
                    ...
                    if (
                        not hasattr(e, "__notes__")
                        or ALREADY_LOGGED_ERROR_NOTE not in e.__notes__
                    ):
                        logger.warn(
            ...
            logger.error(
                f"Error handling client {client_id}:\n{traceback.format_exc()}"
            )
            self._cleanup_client(client)
    ...
    def send_update(self, changes: List[Change], action_id: str):
        ...
        for client_id in messages_for_client:
            ...
            self.send(
                client,
                "update",
                changes=messages_for_client[client_id],
                action_id=action_id,
            )

    def register_message_handler(
        self, message_type: str, handler: Callable[..., None | Awaitable[None]]
    ):
