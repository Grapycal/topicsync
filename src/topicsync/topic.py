...
import abc
from typing import TYPE_CHECKING, Any, Callable, Dict, Generic, Iterable, List, TypeVar
...
def topic_factory(
    ...
    """
    return get_topic_type_from_str(topic_type)(
        name, state_machine, is_stateful, init_value, order_strict
    )
...
class Topic(metaclass=abc.ABCMeta):
    ...
    def _convert_old_format(cls, data):
        ...
        return {"basic": [name, value, is_stateful, order_strict], "additional": None}
...
class StringTopic(Topic):
    ...
    def get_init_message(self):
        return super().get_init_message() | {
            "id": self.version
        }  # client-side SetChange uses 'id' as version

    def _validate_change_and_get_result(self, change: Change):
        # Sometimes StringTopic is used as high freq base64 data. We don't want versioning in that case to
        # save memory and computation.
        if not self._order_strict:
    ...
    def merge_changes(self, changes: List[Change]):
        ...
        for change in changes:
            if isinstance(change, StringChangeTypes.SetChange):
                ...
                while len(stack) and not isinstance(
                    stack[-1], StringChangeTypes.SetChange
                ):
                    stack.pop()
    ...
    def insert(self, position: int, insertion: str):
        change = StringChangeTypes.InsertChange(
            self._name, self.version, position, insertion
        )
        self.apply_change_external(change)

    def delete(self, position: int, deletion: str):
        change = StringChangeTypes.DeleteChange(
            self._name, self.version, position, deletion
        )
        self.apply_change_external(change)
    ...
    def merge_changes(self, changes: List[Change]):
        ...
        for change in changes:
            if isinstance(change, IntChangeTypes.SetChange):
                #  Overwrite all InsertChange or DeleteChange.
                while len(stack) and not isinstance(
                    stack[-1], IntChangeTypes.SetChange
                ):
                    stack.pop()
                if len(stack):  # top is a SetChange
                    if stack[-1].old_value == change.value:  # type: ignore # stack[-1] must be a SetChange
...
class SetTopic(Topic):
    ...
    def notify_listeners(
        self, auto: bool, change: Change, old_value: list, new_value: list
    ):
        ...
        match change:
            ...
            case _:
                raise Exception(
                    f"Unsupported change type {type(change)} for {self.__class__.__name__}"
                )
    ...
    def merge_changes(self, changes: List[Change]):
        ...
        for change in changes:
            if isinstance(change, ListChangeTypes.SetChange):
                #  Overwrite all InsertChange or DeleteChange.
                while len(stack) and not isinstance(
                    stack[-1], ListChangeTypes.SetChange
                ):
                    stack.pop()
                if len(stack):  # top is a SetChange
                    stack[-1].value = change.value  # type: ignore # stack[-1] must be a SetChange
...
class DictTopic(Topic):
    ...
    def notify_listeners(
        self, auto: bool, change: Change, old_value: dict, new_value: dict
    ):
        super().notify_listeners(auto, change, old_value, new_value)
        match change:
